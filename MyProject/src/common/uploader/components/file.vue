...
export default {
  ...
  methods: {
    ...
    fileEventsHandler(event, args) {
      ...
      //fileError 或者 fileSuccess 返回的状态码code和信息message
      let resposeMsg = null
      if (args.length === 4) {
        resposeMsg = JSON.parse(args[2])
      }
      if (this.file === target) {
      ...
      if (resposeMsg) {
        if (resposeMsg.code !== 200) {
          args[0].msgStatus = 'msg-error'
        } else {
          args[0].msgStatus = 'msg-success'
        }
        args[0].msg = resposeMsg.message
      }
    },
    _fileProgress() {
