    ...
    <el-row :gutter="20">
      ...
      <el-col :span="22" style="height: 100%; margin: 0px; padding: 0px;"> 
        <div class='jsontempsidedata'>
          <pre id="result" v-html='jsonData' class='json-context'></pre>
        </div>
      </el-col>
    </el-row>
  </div>
...
export default {
  ...
  methods: {
    ...
    handleNodeClick (data) {
      ...
      if (this.instance && this.instance.status == "UP"){
        vm.$HJloading.open("请求中...");
        let callBack = data => {
          vm.$HJloading.close();
          vm.jsonData = Util.syntaxHighlight(data)
        }
        let failBack = _ => {
          vm.$HJloading.close();
          vm.jsonData = propmt
        }
        Util.requestService({vm, url: '/rpcjson/' + this.instance.id + "/" + data.path, data: this.instance, method: 'post', callBack, failBack})
      } else {
        vm.jsonData = propmt
      }
    }
...
/**内容高度 */
.jsontemp .jsontempsidedata .json-context {outline: 1px solid #ccc; padding: 5px; margin: 5px; width: calc(100% - 30px); height: calc(100% - 50px);overflow-y: auto;}
...
// WEBPACK FOOTER //
// src/components/detailplatform/JSONTemp.vue